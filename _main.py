...
from _services.startup import load_model, log_thread_usage
from _services import validation
from _services.middleware import LoggingMiddleware
from _services.utils import mask_ip
...
async def lifespan(app: FastAPI):
    ...
    app_logger.info(
        "Configured  with %d concurrent worker.", settings.concurrent_worker
    )
...
async def extract(request: Request, text_column: str, file: UploadFile = File(...)):
    ...
    try:
        ...
        results = await loop.run_in_executor(
            request.app.state.executor, extraction_pipeline, model, df, text_column
        )
    ...
    response_content = JSONResponse(
        ...
        content={
            ...
            "data": {
                "positive": {
                    "trend_topics": results["positive_trend_topics"].to_dict(
                        orient="records"
                    ),
                    "frequent_topics": results["positive_frequent_topics"].to_dict(
                        orient="records"
                    ),
                    "count": results["n_positive"],
                },
                "negative": {
                    "trend_topics": results["negative_trend_topics"].to_dict(
                        orient="records"
                    ),
                    "frequent_topics": results["negative_frequent_topics"].to_dict(
                        orient="records"
                    ),
