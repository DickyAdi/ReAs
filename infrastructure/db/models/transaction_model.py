...
class Transactions(Base):
    ...
    id:Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), nullable=False, unique=True, index=True, primary_key=True, default=uuid4)
    idempotency_key:Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), nullable=True, unique=True, index=True) #!SET NULLABLE TO FALSE WHEN REAS ARE READY FOR LAUNCH. SETTING IT TO FALSE JUST TO MAKE IT EASIER TO DEBUG.
    status: Mapped[TransactionStatus] = mapped_column(ENUM(TransactionStatus, name="transaction_status_enum", create_type=True, check_first=True), nullable=False)
    ...
    payment_id: Mapped[UUID] = mapped_column(ForeignKey('payments.id'), nullable=False, unique=True)

    payment: Mapped["Payments"] = relationship("Payments", back_populates='transaction')
    subscription: Mapped["Subscriptions"] = relationship("Subscriptions", back_populates='transaction')
    ...
    def from_entity(cls, transaction:'TransactionEntity') -> "Transactions":
        ...
        transaction.plan_snapshot['created_at'] = transaction.plan_snapshot.get('created_at').isoformat()
        return cls(
