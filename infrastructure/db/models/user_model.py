...
class User(Base):
    ...
    role: Mapped[Role] = mapped_column(
        ENUM(Role, name="user_role_enum", create_type=True, check_first=True),
        default=Role.user,
        nullable=False,
    )
    status: Mapped[UserStatus] = mapped_column(
        ENUM(UserStatus, name="user_status_enum", create_type=True, check_first=True),
        nullable=False,
    ...
    datasets: Mapped[List["Datasets"]] = relationship("Datasets", back_populates="user")
    ...
    def from_entity(cls, user: "UserEntities") -> "User":
        return cls(
            ...
            password=bcrypt.hashpw(
                bytes(user.password.encode("utf-8")), bcrypt.gensalt()
            ).decode("utf-8")
            if user.password
            else None,
