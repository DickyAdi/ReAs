...
from _core.prediction_stats import count_sentiments
...
class ExtractionPipelineFlow:
    ...
    def run(self, df: pd.DataFrame, text_column: str) -> ExtractionPipelineDictType:
        predicted_df, len_valid_mask = self.predictor.run(
            df=df, text_column=text_column
        )
        positive_trend, positive_frequent = self.extractor.run(
            df=df, sentiment="Positive"
        )
        negative_trend, negative_frequent = self.extractor.run(
            df=df, sentiment="Negative"
        )
        if "prediction" not in predicted_df.columns.tolist():
            raise KeyError("`prediction` column does not exists.")
