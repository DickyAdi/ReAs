...
class PredictLogic:
    ...
    def predict(self, df:pd.DataFrame, text_column:str, model:inferenceInterface) -> Tuple[pd.DataFrame, int]:
        if df is None or not isinstance(df, pd.DataFrame):
            raise ValueError(f"Value error: `df` is empty or not pandas DataFrame. Got {type(df)}")
        if not text_column:
        ...
        df.loc[valid_mask, 'prediction'] = model.predict(text_series[valid_mask].tolist())
